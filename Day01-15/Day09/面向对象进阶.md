...
>如果父类没有slots，子类中有slots，则子类中的slots不会起作用； 如果父类和子类都有slots，子类中的slots会覆盖父类的slots
>另外,solts虽然经常被用作封装工具,但他的本意其实为了节省内存和提高访问速度
...
- has-a关系通常称之为关联，比如部门和员工的关系，汽车和引擎的关系都属于关联关系；关联关系如果是整体和部分的关联，那么我们称之为聚合关系；如果整体进一步负责了部分的生命周期（整体和部分是不可分割的，同时存在也同时消亡），那么这种就是最强的关联关系，我们称之为合成关系。
